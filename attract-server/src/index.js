...
const server = http.createServer(async (req, res) => {
    ...
    let latestAttractFileName = attractStats[0][0];
    res.writeHead(200, {
        'Content-Type': 'application/octet-stream',
    });
    const fd = await fs.open(`${attractPath}/${latestAttractFileName}`);
    fd.createReadStream().pipe(res);
});

server.listen(8080, 'localhost', () => console.log('server listening'));
