...
export function ExitDoor(props: { exitDoor: Accessor<ExitDoorData> }) {
    return <g transform={transform(props.exitDoor)}>
        ...
        <path class="exit-door" d={`M ${-13 * props.exitDoor().animProgress} 0 v ${-exitDoorRadius} h ${-exitDoorRadius + exitDoorCorner} l ${-exitDoorCorner} ${exitDoorCorner} v ${2 * (exitDoorRadius - exitDoorCorner)} l ${exitDoorCorner} ${exitDoorCorner} h ${exitDoorRadius - exitDoorCorner} z`} />
        <path class="exit-door" d={`M ${13 * props.exitDoor().animProgress} 0 v ${-exitDoorRadius} h ${exitDoorRadius - exitDoorCorner} l ${exitDoorCorner} ${exitDoorCorner} v ${2 * (exitDoorRadius - exitDoorCorner)} l ${-exitDoorCorner} ${exitDoorCorner} h ${-exitDoorRadius + exitDoorCorner} z`} />
        <path class="exit-door-stroke" stroke-width="3" fill="none" stroke-linecap="round" d={`M ${-13 * props.exitDoor().animProgress} 0 m 0 ${(1 - props.exitDoor().animProgress) * exitDoorRadius} v ${props.exitDoor().animProgress * exitDoorRadius} h ${-exitDoorRadius + exitDoorCorner + props.exitDoor().animProgress} l ${-exitDoorCorner} ${-exitDoorCorner} v ${(1 - props.exitDoor().animProgress) * (-exitDoorRadius + exitDoorCorner)}`} />
        <path class="exit-door-stroke" stroke-width="3" fill="none" stroke-linecap="round" d={`M ${13 * props.exitDoor().animProgress} 0 m 0 ${(1 - props.exitDoor().animProgress) * exitDoorRadius} v ${props.exitDoor().animProgress * exitDoorRadius} h ${exitDoorRadius - exitDoorCorner - props.exitDoor().animProgress} l ${exitDoorCorner} ${-exitDoorCorner} v ${(1 - props.exitDoor().animProgress) * (-exitDoorRadius + exitDoorCorner)}`} />
    </g>;
}
