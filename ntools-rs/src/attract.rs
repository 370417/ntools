...
use crate::{entity::{Entities, boost_pad::BoostPad, bounce_block::BounceBlock, door::{LockedDoor, RegularDoor, TrapDoor}, exit::Exit, floor_guard::FloorGuard, gold::Gold, launch_pad::LaunchPad, mine::Mine, one_way::OneWay, shove_thwump::ShoveThwump, thwump::Thwump}, grid::{COLS, Grid, GridPos, ROWS}, map_file::{MapFile, read_u32}, ninja::Ninja, orientation::{Orientation, OrientationBinary, OrientationCardinal, OrientationExt}, segment::Segment, tile::{Tile, Tiles}};
...
pub fn from_attract_bytes(attract_bytes: &[u8]) -> Result<(MapFile, Vec<u8>), String> {
    ...
    let demo_len = read_u32(&mut cursor).map_err(|_| "failed to read demo len")?;
...
impl Attract {
    ...
    pub fn from_bytes(attract_bytes: &[u8]) -> Result<Attract, String> {
        ...
        for i in 0..num_objects {
            ...
            let _orientation_cardinal = OrientationCardinal::try_from(map_data[i + 3]);
            let orientation_biary = OrientationBinary::from(map_data[i + 3]);
            ...
            match object_id {
                ...
                // Regular door
                5 => entities.doors.regular.push(RegularDoor::new(6.0 * pos, orientation_biary)),
                // O door
                6 => locked_doors.push((6.0 * pos, orientation_biary)),
                // O switch
                7 => locked_switches.push(6.0 * pos),
                // C door
                8 => trap_doors.push((6.0 * pos, orientation_biary)),
                // C switch
